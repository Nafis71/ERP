...
$sql = "SELECT *from machine_list NATURAL JOIN machine_buying_expense where month='$month' and year='$year' and machine_id='$search'";
$run =mysqli_query($connect,$sql);
if(mysqli_num_rows($run) == 0)
{
  header('location:../backend/redirect_searcherror.php?indicate=13');
}
?>
<!DOCTYPE html>
<html lang="en">
 ...
 <!--homesection or main body starts here -->
  ...
  <section class="home-section">
    ...
    <?php
           ...
           $offset = ($page-1) * $limit;
           $query1 = "SELECT *from machine_list NATURAL JOIN machine_buying_expense where month='$month' and year='$year' and machine_id='$search'";
...
<?php echo'<span>Total Entries found '.mysqli_num_rows($result).' & Showing Page Number '.$page.'</span>';?>
      ...
      <thead>
        <tr>
          ...
          <form  method="POST" action="../backend/machinery_expense_excel_search.php">
    ...
    <tbody>
          ...
          <?php
          mysqli_select_db($connect,'erp');
           $query  = "SELECT *from machine_list NATURAL JOIN machine_buying_expense where month='$month' and year='$year' and machine_id='$search' LIMIT {$offset},{$limit}";
...
if(mysqli_num_rows($result)> 0)
{
  ...
  if($page >1)
  {
    echo'<li><a href="../finance/machinery_expense_search.php?page='.($page-1).'&search='.$search.'&month='.$month.'" class="btn btn-primary">Prev</a></li>';
  }
  for($i =1;$i<=$total_page;$i++)
  {
    
    echo'<li><a href="../finance/machinery_expense_search.php?page='.$i.'&search='.$search.'&month='.$month.'" class="btn btn-primary">'.$i.'</a></li>';
  
  }
  if($total_page > $page)
  {
    echo'<li><a href="../finance/machinery_expense_search.php?page='.($page+1).'&search='.$search.'&month='.$month.'" class="btn btn-primary">Next</a></li>';
  }
  echo'</ul>';
